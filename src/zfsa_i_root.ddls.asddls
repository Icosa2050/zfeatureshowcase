...
define view entity ZFSA_I_ROOT
  as select from zfsa_root_a
  association [0..1] to ZFSA_I_CONTACT     as _Contact     on  $projection.ContactID = _Contact.ID
  association [0..1] to ZFSA_I_CRITICALITY as _Criticality on  $projection.CriticalityCode = _Criticality.Code
  association [0..1] to ZFSA_I_NAVIGATION  as _Navigation  on  $projection.NavigationID = _Navigation.ID
